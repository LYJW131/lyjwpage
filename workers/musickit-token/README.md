...
也不能拿站点手上那份凭据来发 —— 那是 Mac 上报器现签的、带 music user token 的
**私人凭据**，拿到就能读我的收听记录，所以它只在服务端用（查跳转链接、拉「最近
在听」），不经任何端点出去。这里要发的是给**任何一个访客**的公开令牌，访客再拿它
去换自己的用户令牌。两者敏感度差一个量级，不共用一条路径。

（站点从前确实有一条把私人凭据发出去的路：`GET /api/ingest/apple-music`，发给那个
在 NAS 上拉列表的上报器。拉列表收回站点之后那条路就没了。）
