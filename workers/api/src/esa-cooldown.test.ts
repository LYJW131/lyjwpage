...
test("ESA 全局 600 秒冷却，突发合并并在没有新上报时补发", async () => {
  const f = fixture();
  try {
    const gate = f.create();
    await Promise.all(Array.from({ length: 20 }, () => gate.request()));
    assert.deepEqual(f.sent, [1000]);
    assert.ok(f.scheduled.every(at => at === 601000));
    f.at(600999); await gate.flush(); assert.equal(f.sent.length, 1);
    f.at(601000); await gate.flush(); assert.deepEqual(f.sent, [1000, 601000]);
    f.at(1201000); await gate.flush(); assert.equal(f.sent.length, 2);
  } finally { f.db.close(); }
});
...
test("冷却与待刷新状态跨对象重建保留，重复 alarm 不会重复发送", async () => {
  ...
  try {
    ...
    f.at(601000); await Promise.all([restarted.flush(), restarted.flush()]);
    assert.deepEqual(f.sent, [1000, 601000]);
    f.at(610000); await restarted.request();
    assert.equal(f.scheduled.at(-1), 1201000);
  } finally { f.db.close(); }
});
...
test("发送未完成及失败时仍遵守间隔，期间的新变化保留待发", async () => {
  ...
  try {
    ...
    f.at(601000); await f.create().flush();
    assert.deepEqual(f.sent, [1000, 601000]);
  } finally { f.db.close(); }
});
