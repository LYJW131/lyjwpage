...
公开 API 为 `/api/status/*`、`/api/home`、`/api/lyrics`、`/api/motion-artwork`。浏览器挂载后直接访问这里，Vercel 只在首屏生成或重建时读取 `/api/home`。服务端凭据不进入任何公开响应，没有通用 HTTP 数据库端点。`GET /api/status/activity-history` 返回五域活动历史（`?since=` 增量），不进 `/api/home`、不推送；与手表圆环 `/api/status/activity` 无关。
