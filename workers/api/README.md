...
一轮实测要二十多秒，读路径一步都不去跑上游：由 cron 每分钟检查、用 `claim` 抢到当前这一小时的才实测，
跑完并进窗口、重算中位数再写进缓存，`/api/status/vercel-deployments` 只读已经算好的那份。抢到之后失败不回滚，
下一轮等到下个小时；缓存和样本历史各保留一天，连续失败超过一天才回到 `—`。没配 `PAGESPEED_API_KEY` 就整段跳过。
