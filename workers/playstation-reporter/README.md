...
开着就 2 分钟一轮完整 tick，一个都没开就 15 分钟一轮。presence 每轮都推（站点靠它的
...
于是站点有页面开着时是 2 分钟一轮；一个都没开时每分钟的 cron 把 14.5 分钟这个阈值
...
切回来看」这个真正的判据。代价是快档命中的时长长得多，所以快档从 60 秒放宽到
2 分钟：presence 本来就没有秒级意义，而闲挂一天的标签页按 60 秒算就是一天 1440 次
PSN 请求。

快档为什么是 115 秒不是 120：cron 每分钟一响，实际节奏只能落在 60 / 120 / 180 秒
这个网格上。卡整数的话早响半秒的那一响会被门挡掉，实际退化成 3 分钟一轮 —— 留
5 秒取整余量，第 120 秒那一响才稳稳够格。
