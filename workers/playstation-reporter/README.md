...
Cloudflare Worker 上的 PSN 上报器：cron 每分钟响一次，前面挡一道门 —— 站点有页面
开着就 60 秒一轮完整 tick，一个都没开就 15 分钟一轮。presence 每轮都推（站点靠它的
...
- 攒够 14.5 分钟就直接放行，连连接数都不问 —— 闲时节奏不该依赖另一个 worker 可不可达；
- 不到 55 秒直接挡回去；
- 中间那段才去问 `live-push` 的 `GET /count`（超时 2.5 秒）：连接数大于 0 就放行。

于是站点有页面开着时是 60 秒一轮；一个都没开时每分钟的 cron 把 14.5 分钟这个阈值
取整成 15 分钟一轮，**和从前那根十五分钟的 cron 逐轮对齐，闲时对 PSN 的流量一模
一样**。动机就是这个：presence 的新鲜度跟着「有没有人可能看」走，没人开着的时候
一分钱不多花。

**为什么读 live-push 而不是 online-counter。** 后者那条连接在页面进后台时是整条
关掉的，数出来是「此刻可见」的页面 —— 切个标签页、锁个屏就掉成 0，门跟着来回抖。
live-push 那条一直挂着，数的是「开着」的页面（含后台标签页、锁了屏的手机），这才
对得上「有人可能切回来看」这个真正的判据。代价是快档命中的时长长得多：一个标签页
挂着不关就是一天 1440 次 PSN 请求 —— 明摆着的取舍，换的是切回页面时 presence 最坏
只旧 60 秒。

快档阈值写 55 秒不写 60：cron 每分钟一响，卡整数的话早响半秒的那一响会被门挡掉、
实际退化成两分钟一轮，留 5 秒取整余量才稳稳够格。

**live-push 一份生产一个，`LIVE_PUSH_URL` 目前只填 Vercel 那一份**（根 README
「两份生产之间靠传播上报对齐」）。EdgeOne 那份生产上的访客因此暂时数不进来 ——
偏差方向和上面那条兜底一致：少数了只会让门退回慢档，不会误提速。

门里只有那两个读操作，并且排在任何贵操作之前 —— 被挡下的那一响完全不碰 PSN、
不碰站点。连接数接口超时、非 200、返回形状不对，一律**当 0 处理**：兜底方向是单向的，
读不到只会退回 15 分钟的基线，永远不会因为故障变快。

KV 的读带最长 60 秒的边缘缓存，而门的阈值就在这个量级上，所以门还看一眼 isolate
...
`LIVE_PUSH_URL` 同样配好了，填的是 live-push worker 的**源**（路径由这边拼 `/count`，
和站点侧 `NEXT_PUBLIC_LIVE_PUSH_URL` 同一个形状）。注释掉它不会让上报停摆，只是门
永远按「一个页面都没开」走，退回 15 分钟一轮的基线节奏。
