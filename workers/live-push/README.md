...
「不计数」和「关掉」是两条线，不是一条：手机锁屏、移动端后台标签页会被浏览器
**整个冻结**，定时器完全停掉 —— 这样的页面此刻确实收不到也画不出来，不算数是对的，
但它随时会解冻回来，关掉只会逼它重连一次。所以关的那条线推到 **30 分钟**，只收
真正回不来的，顺路在数人头时做掉，不额外挂闹钟（定时唤醒实例会把休眠省下的东西
抵消掉）。
