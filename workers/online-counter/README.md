...
**这个数是「此刻可见」的页面数，不是「开着」的页面数。** 客户端在页面进后台时是
整条连接关掉（见下面清理那节），所以切个标签页、锁个屏，这里立刻少一个。页面上要
显示的就是这个口径。上报器的调频门读的**不是**这里，是 `live-push` 的 `/count`
（那份数的是开着的页面，含后台）—— 理由见 `workers/live-push/README.md`。
...
顶高。这个数是印在页面上给人看的，虚高就是直接说错话，而且每条日志看着都合法。
...
- 清扫由 Durable Object 的闹钟驱动，每 30 秒一次，房间里还有人就续订、人走光了
  ...
  那一圈闹钟什么也不做。
- `GET /count` 会先清扫再回答，读到的是读的那一刻的准数，不用等下一次闹钟。
  但只靠这条惰性清扫是不行的：页面上那个数走的是广播、不经过 `/count`，而 `/count`
  可能整天没人读。自家计数的正确性得由闹钟兜住。
