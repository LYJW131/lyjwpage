...
| 最近 12 小时的上报轮数与平均 CPU（`window`） | 每轮追加一条、丢掉 12 小时前的，和流量累计存在同一个状态文件里 | 跟着一起推，站点卡片 misaka-jp 那格用它，和 Vercel / Workers 的 12 小时窗口对齐。CPU 按每段时长加权；刚起来时窗口不满 12 小时，`start` 说明实际从哪算起。攒不住时同样报 `null` |
