...
| 最近 12 小时的平均 CPU 与进出字节（`window`） | 每轮追加一条、丢掉 12 小时前的，和流量累计存在同一个状态文件里 | 跟着一起推，站点卡片 misaka-jp 那格用它，和 Vercel / Workers 的 12 小时窗口对齐。CPU 按每段时长加权；刚起来时窗口不满 12 小时，`start` 说明实际从哪算起。攒不住时同样报 `null` |
| 镜像的提交（`reporterCommit`） | 镜像构建时由 Actions 以 `GIT_SHA` 烧进 `REPORTER_COMMIT` | 跟着一起推，站点卡片显示线上跑的是哪一版。本地直接 `python3 reporter.py` 时为 `null` |
