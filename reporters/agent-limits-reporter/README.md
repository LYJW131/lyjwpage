...
| `CURSOR_NOW_LIVE_INTERVAL_MS` | | 默认 `300000`，有可见页面时查 Cursor 最近用量事件的间隔；站点灯的 10 分钟窗口按它放宽，改时一起改 |
| `CURSOR_NOW_OPEN_INTERVAL_MS` | | 默认 `600000`，只有后台页面 |
...
Cursor 那盏「在用」的灯另起一条快循环（`src/cursor-now.ts`）：同一份会话 cookie 查最近 15 分钟的用量事件，
