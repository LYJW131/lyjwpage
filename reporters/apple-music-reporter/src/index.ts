...
/**
 * 此刻挂在 live-push 上的连接数，也就是**开着**本站的页面数（含后台标签页、
 * 锁了屏的手机）。超时、非 200、形状不对，一律当 0。
 *
 * 兜底方向是单向的：读不到只会让节奏退回基线，永远不会因为故障变快 —— 认错这个
 * 方向的代价是一个页面都没开的时候仍按快档一直打 Apple。
 */
async function liveConnections(): Promise<number> {
  // 没配这个变量不是故障，别让它进 failure 的连击计数
  if (!config.liveCountUrl) return 0;

  try {
    const response = await fetch(config.liveCountUrl, {
      signal: AbortSignal.timeout(config.liveCountTimeoutMs),
    });
...
info(
  `apple-music-reporter 启动：api.music.apple.com → ${config.site.ingestUrl}，每 ${config.recentIntervalMs / 1000} 秒一轮${
    config.liveCountUrl ? `（站点有页面开着时 ${config.liveIntervalMs / 1000} 秒）` : ""
  }`,
);
