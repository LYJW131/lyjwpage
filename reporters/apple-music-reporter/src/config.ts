...
export const config = {
   ...
   * 2 分钟，这是唯一有人可能看见的那段时间。
   *
   * 从前这里是 30 秒，配的是「此刻有人正看着」那个口径。门改读 live-push 的
   * 连接数之后，「有人」变成了「页面开着」（含后台标签页、锁了屏的手机），命中的
   * 时长长得多，30 秒会把闲挂一天的标签页也变成一天 2880 次 Apple 请求。放宽到
   * 2 分钟是这个口径下的重新配平，和 playstation-reporter 那档取同一个数。
   *
   * 改这一档不用动站点的 `LISTENING_STALE_MS`：那个窗口锚的是闲时那档 —— 有页面
   * 开着时只会更快，判活的下限始终由慢的那档定。
   */
  liveIntervalMs: ms("LIVE_INTERVAL_MS", 120_000),
