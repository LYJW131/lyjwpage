...
| 内容 | 节奏 | 什么时候真的推 |
| --- | --- | --- |
| 最近播放列表（含封面、链接、首项时长） | 站点有页面开着 60 秒一轮 / 一个都没有 15 分钟 | 内容有变化时；另外每 10 分钟兜底整推一次（闲时每轮都已到期，等于 15 分钟一封） |
| 「此刻在不在听」的推断 | 跟着上面那轮 | 同上，它是同一份载荷里的一个字段 |

每轮收尾时问一次 `live-push` 的 `GET /count`（超时 2.5 秒），连接数大于 0 就按快档
睡下去；一个都没有的话，闲档那一觉**拆成一个个快档长度的小觉**，每觉醒来再问一次，
问到人就立刻开跑 —— 一睡 15 分钟不回头的话，访客打开页面之后最坏要等一刻钟才见到
新数据，而缩短这段等待正是快档存在的理由。多打的那些请求打的是自己的 worker（不是
Apple），而 `playstation-reporter` 的 cron 闲时本来就差不多每分钟问它一次。15 分钟那一档只在**一个页面都没开**的时候才划算 —— 那时这份轮询只为记录
历史而跑，晚一会儿落地没人受影响，换来的是闲时对 Apple 的请求从 1440 次/天降到
96 次/天；有页面开着的时候多打几次 Apple，换歌延迟从最坏 15 分钟降到 60 秒。
接口超时、非 200、返回形状不对，一律**当 0 处理**：兜底方向是单向的，读不到只会退回
15 分钟的基线，永远不会因为故障变快。不配 `LIVE_PUSH_URL` 就恒走基线。

**为什么读 live-push 而不是 online-counter。** 后者那条连接在页面进后台时是整条
关掉的，数出来是「此刻可见」的页面 —— 切个标签页、锁个屏就掉成 0，档位跟着来回抖。
live-push 那条一直挂着，数的是「开着」的页面（含后台标签页、锁了屏的手机），这才
对得上「有人可能切回来看」这个真正的判据。代价是快档命中的时长长得多：一个标签页
挂着不关就是一天 1440 次 Apple 请求 —— 明摆着的取舍，快档和 `playstation-reporter`
那档取同一个数。

**live-push 一份生产一个，`LIVE_PUSH_URL` 目前只填 Vercel 那一份**（根 README
「两份生产之间靠传播上报对齐」）。EdgeOne 那份生产上的访客因此暂时数不进来 ——
偏差方向和上面那条兜底一致：少数了只会退回慢档，不会误提速。

兜底整推那 10 分钟是按墙钟判定的，但判定写在 tick 里 —— **只有 tick 跑起来才检查**，
而闲时一轮就是 15 分钟，每轮都早过了阈值。所以闲时整推和轮询合一，站点固定每
15 分钟收一封。站点侧的断流窗口（`src/lib/freshness.ts` 的 `LISTENING_STALE_MS`，
50 分钟）锚的就是这个闲时节奏：改**闲那一档**必须同步改那边，改快档不用。
...
| `RECENT_INTERVAL_MS` | | 默认 `900000`，一个页面都没开时那一档。这也是「换歌时刻」的观测精度，站点的 `LISTENING_STALE_MS` 锚着它 |
| `LIVE_INTERVAL_MS` | | 默认 `60000`，站点有页面开着时那一档 |
| `LIVE_PUSH_URL` | | live-push worker 的**源**，路径由这边拼 `/count`，和站点侧 `NEXT_PUBLIC_LIVE_PUSH_URL` 同一个形状。不配就恒走基线 |
| `LIVE_COUNT_TIMEOUT_MS` | | 默认 `2500`，问连接数的超时 |
...
- 换歌时刻最多晚一个**当前档位的**轮询间隔被记下：有页面开着时 60 秒，一个都没开
  时最坏 15 分钟 —— 那段时间里这份记录只进历史，没人正盯着它翻；
