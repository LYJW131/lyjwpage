...
| 内容 | 节奏 | 什么时候真的推 |
| --- | --- | --- |
| 最近播放列表（含封面、链接、首项时长） | 有观众 20 秒一轮 / 没人 60 秒 | 内容有变化时；另外每 10 分钟兜底整推一次 |
| 「此刻在不在听」的推断 | 跟着上面那轮 | 同上，它是同一份载荷里的一个字段 |

每轮收尾时问一次 `online-counter` 的 `GET /count`（超时 2.5 秒），有人在线就按快档
睡下去。60 秒那个折中只在**没人看**的时候才需要成立 —— 有人正看着的这几分钟里多打
几次 Apple，换来换歌延迟从最坏 60 秒降到 20 秒。人数接口超时、非 200、返回形状不对，
一律**当 0 处理**：兜底方向是单向的，读不到只会退回 60 秒的基线，永远不会因为故障
变快。不配 `ONLINE_COUNTER_URL` 就恒走基线。

兜底整推那 10 分钟不受影响，两档共用同一个墙钟判定 —— 站点侧的断流窗口
（`src/lib/freshness.ts` 的 `LISTENING_STALE_MS`）锚的就是它，和轮询间隔无关，改这边
不用动那边。
...
| `RECENT_INTERVAL_MS` | | 默认 `60000`，没人看时那一档。这也是「换歌时刻」的观测精度 |
| `LIVE_INTERVAL_MS` | | 默认 `20000`，有观众时那一档 |
