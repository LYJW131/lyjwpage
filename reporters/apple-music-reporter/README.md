...
每个访客的每一轮轮询都要重走一遍缓存（一次请求十几趟 Redis）。现在站点那侧只剩
一次 Redis 读。
...
实测 developer token 寿命约 **29 天**，所以这个上报器平时不依赖 Mac 在线，只在
每月重签那一次要 Mac 醒着一回。到期前一小时会主动去换新的；换不到但旧的还没过期
...
```bash
COPYFILE_DISABLE=1 tar czf - -C reporters --exclude node_modules --exclude dist apple-music-reporter | ssh dsm 'mkdir -p /volume3/docker && tar xzf - -C /volume3/docker'
```

`.env` 单独送，别混进源码目录一起打包：

```bash
ssh dsm 'cat > /volume3/docker/apple-music-reporter/.env && chmod 600 /volume3/docker/apple-music-reporter/.env' < 本机那份.env
```

起：

```bash
ssh dsm '/usr/local/bin/docker compose -f /volume3/docker/apple-music-reporter/compose.yaml up -d --build'
```
