...
| 内容 | 节奏 | 什么时候真的推 |
| --- | --- | --- |
| 最近播放列表（含封面、链接、首项时长） | 站点有页面开着 2 分钟一轮 / 一个都没有 15 分钟 | 内容有变化时；另外每 10 分钟兜底整推一次（闲时每轮都已到期，等于 15 分钟一封） |
...
96 次/天；有页面开着的时候多打几次 Apple，换歌延迟从最坏 15 分钟降到 2 分钟。
...
live-push 那条一直挂着，数的是「开着」的页面（含后台标签页），这才对得上「有人可能
切回来看」这个真正的判据。代价是快档命中的时长长得多，所以快档从 30 秒放宽到
2 分钟（和 `playstation-reporter` 那档同一个数）：闲挂一天的标签页按 30 秒算就是
一天 2880 次 Apple 请求。
...
| `LIVE_INTERVAL_MS` | | 默认 `120000`，站点有页面开着时那一档 |
...
- 换歌时刻最多晚一个**当前档位的**轮询间隔被记下：有页面开着时 2 分钟，一个都没开
