 ...
 *
 * 三份上报器各拷这一份（apple-music / emby / playstation），不抽公共包：各自是
 * 独立的部署单元，三十行拷过去比为它们建一个要一起版本管理的包省事。
 *
 * 这一份还多一条纪律：**任何 token 的内容都不许进日志**。PSN 那边的 access /
 * refresh token 和换码用的 access code 都是明文字符串，随手 console.log 一个
 * 响应体就会把它们写进 docker logs 里躺着。上游报错时只带状态码和截断过的正文，
 * 拿到 token 之后只说到期时刻。
 */
