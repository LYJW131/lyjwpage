...
身体活动沿用 iPhone 累计快照生成的有界区间，跨日、长间隔和回退不填补。已完成训练另从 `workouts:recent` 进入同一套 Jev 判据：项目名和摊到五分钟窗口里的活动秒数与圆环估算并列，圆环桶不混入这笔时长。训练覆盖而圆环没有样本的窗口也会打分。
...
原始状态继续按现有水位归档 D1；模型评分在 StateHub 保留七天，不混入原始状态表。
发布顺序为 Worker、前端契约生效后再安装 Mac 采集器。
