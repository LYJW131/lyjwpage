...
- 跨域活动历史键为 `activity-history:<domain>`（`coding` / `listening` / `watching` / `gaming` / `charging`）。每域最多 600 条，TTL 7 天；同水平非空闲最多每 5 分钟再确认一次，空闲只留一条。读出口 `GET /api/status/activity-history?since=`，给将来的活动评分图当底。不推送，不进 `/api/home`。
