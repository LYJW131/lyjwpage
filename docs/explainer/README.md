...
Claude Code 的像素螃蟹 Clawd 讲解这个站点怎么运转：10 章、130 小节（100 BPM，约 5 分 13 秒）。画面、配乐和音效都在浏览器里按同一条时间轴确定地生成，同一个时刻总是同一帧。分镜与旁白见 [SCRIPT.md](SCRIPT.md)。
...
生成物不进仓库：`music-{chip,piano,lofi,pluck}.mp3` 和发布用的 `artifact.html`（见根目录 `.gitignore`），按下面的步骤重新生成。

## 预览

```bash
node docs/explainer/render/serve.mjs docs/explainer 4817
```

打开 `http://localhost:4817`（`.claude/launch.json` 里的 `explainer-preview` 是同一条命令）。这个静态服务支持 Range 请求，拖进度、换配乐后跳回原位置都依赖它。
...
```bash
H="$PWD/docs/explainer/index.html"; R=docs/explainer/render; O=/tmp/explainer
...
cp $O/mix-chip.mp3 docs/explainer/music-chip.mp3
```
...
node $R/edge2.mjs http://localhost:4817/               # 加载失败回退、放完重播、触屏底栏、封面（要先起预览）
...
`render/build-artifact.py` 从 `index.html` 生成 `artifact.html`，去掉 doctype/html/head/body 外壳。发布时把它和脚本、字体、四个 mp3 一起发布为 claude.ai 上的私有 Artifact。
