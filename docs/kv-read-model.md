...
| `/api/home` | DO；Vercel 生成或重建首页读一次，浏览器打开页面后各卡的第一轮取数合成一次（`src/lib/home-bootstrap.ts`） | 不经 KV |
...
`/api/home` 不进 KV，两次 bootstrap 都直读 DO：Vercel 生成或后台重建首页时读一次；浏览器挂载后各卡的第一次回源合成一次，之后各卡按自己的周期直连各自端点。挂载这一次不能关：「此刻」类信封里有服务端按当时时钟算的结论（存活、日界线、暂停宽限期），HTML 放一会儿就不成立。曾经试过让挂载这次读 KV 投影并按时间戳挡掉比首屏旧的那份：首页重建是访问触发的，快照时刻等于上一次访问，KV 投影几乎总显得更旧，守卫每次都触发、各卡照旧十几个请求，于是改回直读 DO，一次聚合渲染的代价和从前十几个端点各读一次相当。两条保护在 `home-bootstrap.ts`：本页收到过推送或失效通知的路径不吃聚合（登记覆盖全部推送路径，不只是走 KV 的三条）；只服务打开页面后 15 秒内的第一次取数。聚合请求 5 秒超时、非 2xx、字段缺失或该字段本身 `ok:false` 都退回直连。带 `since` 的增量请求也吃聚合（合并器本来就接受整份），其他查询参数不碰。
