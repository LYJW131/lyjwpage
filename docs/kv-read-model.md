...
3. 每分钟已有 cron 也标记全部公开路径，覆盖 Apple 最近播放、外部查询缓存、只随时间变化的视图，以及给已有 DO 新增 KV binding 的首次回填。
4. DO alarm 从 `public_read_model_jobs` 取到期项，调用现有公开 API 生成结果，每轮最多处理三个。只有这个对象向对应 KV 键发布；边缘读 miss 仅请求补建，绝不把自己先前读到的旧响应回写 KV。
