...
/**
 * Apple Music 上报器每轮收尾问一次人头数，据此选档：有页面可见 60 秒一轮，只是
 * 开着（后台标签页、锁了屏）2 分钟一轮，一个页面都没开 15 分钟一轮。10 分钟一次的兜底整推确实是按墙钟判定的，但那个判定写在 tick
 * 里——**只有 tick 跑起来才检查**，而闲时一轮就是 15 分钟，每轮都早过了阈值。
 * 所以没人看时整推和轮询合一，实际心跳节奏就是闲时那一档的 15 分钟，这个窗口
 * 锚的是它。三轮多一点：漏一两轮不该让卡片翻脸，连着三轮没到才算它出事。
 *
 ...
 * 上报器那侧改**无人档**间隔
 * （`IDLE_INTERVAL_MS`）时这里要跟着改，改另外两档不用 —— 有人看时只会更快，
 * 判活的下限始终由 15 分钟那档决定。
 */
