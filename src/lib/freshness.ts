...
/**
 * Apple Music 上报器每轮收尾问一次 live-push 的连接数，据此选档：站点有页面开着
 * 2 分钟一轮，一个都没有 15 分钟一轮。10 分钟一次的兜底整推确实是按墙钟判定的，
 * 但那个判定写在 tick 里——**只有 tick 跑起来才检查**，而闲时一轮就是 15 分钟，
 * 每轮都早过了阈值。所以闲时整推和轮询合一，实际心跳节奏就是闲时那一档的
 * 15 分钟，这个窗口锚的是它。三轮多一点：漏一两轮不该让卡片翻脸，连着三轮没到
 * 才算它出事。
 *
 * 45 分钟之外还要再宽一截给缓存：端点读的是 'use cache' 那份快照，而内容没变的
 * 那次整推**不发失效通知**（只有 changed 才带 tag），只能等快照自己到期，拿到手的
 * pushedAt 可能比 Redis 里那份旧一个刷新周期。3 × 15 = 45，留到 50。
 * 上报器那侧改**闲档**间隔
 * （`RECENT_INTERVAL_MS`）时这里要跟着改，改快档不用 —— 有页面开着时只会更快，
 * 判活的下限始终由 15 分钟那档决定。
 */
export const LISTENING_STALE_MS = 50 * 60_000;
 ...
 *
 * 一轮多久要看站点还有没有页面开着：Worker 的 cron 每分钟响一次，但门只在
 * live-push 的连接数大于 0 时放行到 2 分钟一轮，一个都没有就压回 15 分钟一轮。
 * 所以这个窗口锚的是**闲时**那档 —— 有页面开着时只会更快，判活的下限始终由
 * 15 分钟那档决定。
 *
