 ...
 *
 * 每人的 `weeks` 和顶层 `weeks` 共用同一组 weekStart（窗尾对齐），空周补零，
 * 柱状图横轴才能并排对齐。
 */
export function summarizeRepoStats(
  ...
  for (const entry of raw) {
    for (const week of Array.isArray(entry.weeks) ? entry.weeks : []) {
      ...
      if (start) weekStarts.add(start);
    }
  }
  const allStarts = [...weekStarts].sort((left, right) => left - right);
  const windowStarts = allStarts.slice(Math.max(0, allStarts.length - Math.max(1, weekWindow)));
...
export async function fetchRepoStats(
   ...
   * 多等几轮仍没有，就退到 commits 列表按作者/周聚合——没有 ++/--，但柱状图和
   * 排名还能画，本地和 Worker 都不至于整卡空白。
   */
  const attempts = Math.max(1, Math.min(12, Math.floor(maxAttempts)));
  const headers = {
  ...
  return fetchRepoStatsFromCommits(token, owner, name, headers);
}
...
function weekStartSeconds(iso: string): number | null {
  ...
  return Math.floor(utc / 1000) - day * 86_400;
}
...
async function fetchRepoStatsFromCommits(
  ...
  void token;
  const since = new Date(Date.now() - WEEK_WINDOW * 7 * 86_400_000).toISOString();
