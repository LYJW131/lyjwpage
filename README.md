...
| 事件 | 形状 |
| --- | --- |
| `desktop` · `listening-now` · `watching-now` · `charger` | 带数据，浏览器直接写进 SWR 缓存 |
| `listening` · `watching` · `presence` | 只发失效通知（payload 为 `null`），浏览器自己回来取 |

两张列表走失效通知而不是带数据：整份十几 KB，而浏览器手上多半只差一两项，让它自己回来取一次更省。它们的轮询兜底因此放到了 5 分钟一轮 —— 即时性由推送负责，轮询只兜「推送整体停用」这一种情况。
