...
> 卡片的「在 Emby 里打开」跳转链接指向 `EMBY_PUBLIC_URL`，生产上是 **只读展示站**
> `emby.lyjw.dev`（部署单元在 [emby-public/](emby-public/)）：访客打开即以 `guest` 浏览元数据，
> 库范围和 `example` 用户一致，不能播放、不能下载、不能改状态。从前指向 `emby.lyjw131.com`，
> 那边挂着 authentik，跳过去的人会撞到登录页，链接等于点不开。真正要播放仍去 `emby.lyjw131.com`。
> 展示站的安全前提是**访客浏览器里没有真 token**（Emby 的用户策略拦不住直出字节的接口，
> 详见那边的 README），别把它改回「发一个真 token 给访客」的做法。没配这个变量就不给链接，
> 没有内网地址可退，退了也是个点不开的链接。
