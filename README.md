...
条目手写在 `src/lib/changelog.ts`，**不从 `git log` 生成**。两个理由，都在那个文件顶上写了：Vercel 是浅克隆、EdgeOne 的克隆深度也不由这个仓库决定，`git log -50` 在三份部署上会给出不同长度甚至空的结果，而且页面上看不出来它是错的（`git rev-parse HEAD` 深度 1 就够，所以页脚那个 sha 没事）；再者提交信息不是给访客看的话，50 个提交里 `docs` / `style` / `refactor` 就占了 13 个。**摘要手写、真相在 git**：每条填一个 `commit`，渲染时和上一条拼成 GitHub 的 compare 链接。

发一条新的就是在数组最前面加一项：

```ts
{
  version: 10,          // 递增整数。新旧判定只比这一个值
  date: "2026-08-20",   // 展示用字符串，不是时间戳，别拿它去 new Date()
  title: "一句话说清这次改了什么",
  notes: [{ kind: "new", text: "..." }],  // new 新增 / fix 修复 / tune 优化
  commit: "abc1234",    // 这条覆盖到哪个提交为止；起点自动取上一条的
}
```

`kind` 只有三种，且按**访客看得见的影响**分，不照搬 conventional commit 那七八种 —— `refactor` 和 `perf` 对着页面看是同一件事。顺序、日期、sha 格式这些不变量由 `src/lib/changelog.test.mts` 拦着，`pnpm test` 会跑。

**触发的钥匙是条目的 `version`，不是构建号。** 按 `COMMIT_SHA` 判定的话每次部署都会弹一次，包括改个 README、调一格间距 —— 弹出来里面没有值得看的东西，第二次之后就没人再看了。所以只有真写了新条目才提示；构建号照旧显示在面板底部和页脚。

浏览器把「看到第几条」存在 `localStorage` 的 `changelog-seen` 里，一个数字。**第一次来的人不提示**：没有记录时静默写入当前进度，他没有「上一次」。存不进去（无痕模式）就一直不提示，比每次进来都弹一条强。

页脚上方那条窄条只在有没看过的条目时出现；「都看过」之后入口只剩页脚那行里的**更新日志**，所以别把它删了。面板是原生 `<dialog>` + `showModal()`，焦点陷阱、Esc、`::backdrop` 都是浏览器自带的，没引任何弹窗库。

一件**没做**的事：标签页开着不动的时候，新部署上线它是不知道的（这个站的标签页会开一整天）。要做的话是给 live-push 加一种 `build` 事件 —— 那条链路上已经有 `presence` 这种没有对应 status 端点的事件先例 —— 但需要一个发布方（部署钩子 → 一条路由 → `publish`），而且两份生产各要接一次。
