...
海报由 Emby 上报器一次压成 WebP，以 `<sha256>.webp` 直传 R2；站点只接收对象键并保存公开桶直链（`R2_PUBLIC_BASE_URL`）。地址即内容指纹，所以对象带 `max-age=31536000, immutable`，浏览器直连 R2 取图，站点没有图片读写或转码路径。
...
- **上报器直传图片**：Mac 图标和 Emby 海报在上报器侧一次压成 `<sha256>.webp` 并直传 R2。站点只 HEAD 校验对象键并保存公开 URL，不读取、不压缩也不写图片字节；Redis 里不存任何图片二进制。
...
Music.app 封面和前台应用图标由 Mac 直接读取，只在内容变化时上传一次。配置本机 R2 凭据后，前台应用图标由 Mac 直接 PUT 到 R2，网站只接收对象哈希；未配置时仍兼容服务端接收并写入 R2。状态里只存公开直链，普通状态心跳不会重复携带图片。时区模块只上传 IANA 标识、当前偏移和缩写，不上传地址。公开读取按用途拆分为 `/api/status/charger`、`/api/status/desktop`、`/api/status/timezone`、`/api/status/music` 和 `/api/status/vibecoding`。
