...
直传 R2，在线人数、调频用的连接数和令牌则是浏览器、上报器直连 Worker。交互版是自包含的一页
[`docs/architecture.html`](docs/architecture.html)（GitHub 不渲染 HTML，克隆下来用浏览器打开），
节点上标着对应的源码位置，还能按上报入库 / 两份生产互转 / 浏览器三条腿 / 连接数反馈环 /
...
上报器拉 `/v1/me/recent/played?limit=10`，站点有页面开着时 2 分钟一轮、一个都没开时 15 分钟一轮。注意这个端点返回的是**专辑、歌单、电台这类容器**，不是单曲：专辑给 `artistName`、歌单给 `curatorName`，没有 `durationInMillis`，`limit` 上限是 10。时长要顺着容器的 `href` 再查一次曲目加起来，封面对自建歌单还要去资料库副本取——这些查询的缓存全在上报器进程里，站点的 Redis 不再存它们。
...
**每封都是心跳。** 上报器 30 秒一轮、每轮都发，站点拿 `pushedAt` 判断它还活着没有。
断流窗口默认 90 秒（三倍间隔，可用 `SERVER_STALE_MS` 改）。这条路上没有实时推送 ——
CPU 和网速每个间隔都在变，广播就是拿推送当轮询用；卡片 30 秒自己来问，上报只让首屏
那份缓存失效。间隔从 15 秒放宽到 30 秒：那本来是全站最快的一路，而状态端点是
`no-store`、每次都进函数。改的顺序和心跳窗口那条一样 —— 先放宽窗口部署，再给上报器降频。
