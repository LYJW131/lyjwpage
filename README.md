...
| **Pulse** | 编码、听、看、玩、充电、身体活动六个域最近 24 小时的活跃度泳道，听、看、玩曲线显示二值状态，充电曲线显示实测瓦数；六项右侧均保留 Jev 评分和趋势。身体活动泳道另外按时间标出这 24 小时内完成的训练项目名。 |
...
**Pulse**：六个域最近 24 小时的活跃度泳道，听、看、玩曲线直接显示播放／游戏状态，充电曲线和详情显示实测瓦数。六项右侧均由 Jev 五分钟评分汇总强度、趋势和置信度；历史每分钟归档到 D1。身体活动泳道在圆环估算之外，把 `/api/status/workouts` 里落在这 24 小时内的训练按开始时间标上项目名（如 Fencing），不送进 Jev。
